...
func TestRequestLogger_SendsLogEntryOnFinish(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/payments", bytes.NewBufferString(`{"order_id":1}`))
	...
	if entry.Source != (logclient.LogSource{AppName: "payment-service", ServiceName: "payment"}) {
		t.Errorf("Source = %+v, want {payment-service payment}", entry.Source)
	}
	if entry.TraceID == "" {
		t.Error("TraceID is empty, want a generated id")
	}
	if entry.Endpoint != "/payments" {
		t.Errorf("Endpoint = %q, want /payments", entry.Endpoint)
	}
	if entry.HTTPStatus != "201" {
		t.Errorf("HTTPStatus = %q, want 201", entry.HTTPStatus)
	}
